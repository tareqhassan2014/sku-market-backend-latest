...
exports.updateProduct = catchAsyncErrors(async (req, res, next) => {
    ...
    if (req.body.stores && req.body.stores.length > 0) {
        const sellers = [
            ...new Set(req.body.stores.map((store) => store.store_id)),
        ];

        const stores = req.body.stores.map((store) => {
            // find by id and update the store some times some fields are not sent in the request body so we need to update the store with the old values
            return Store.findOneAndUpdate(
                { product: req.params.id, store_id: store.store_id },
                {
                    ...store,
        ...
        // wait for all the promises to be resolved
        const store_id = await Promise.all(stores);
...
exports.searchProductsPaginate = catchAsyncErrors(async (req, res, next) => {
    ...
    const marketplace = req?.query?.sku_marketplace;
