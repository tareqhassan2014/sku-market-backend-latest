...
const { SellerProduct } = require("./userProduct.controller");
...
exports.createPortfolio = catchAsyncErrors(async (req, res, next) => {
  ...
  if (portfolios >= 5) {
    return next(new AppError("You can only have 5 portfolios. Delete one to add a new one.", 400));
  }
...
exports.getPortfolios = catchAsyncErrors(async (req, res) => {
  ...
  const portfolios = await Portfolio.find({ user }).select("store_id sku_marketplace");
  res.status(200).json({
    status: "success",
    data: portfolios,
  });
...
exports.removeProductFromPortfolio = catchAsyncErrors(async (req, res, next) => {
  ...
  const updatedProducts = portfolio.store_products.filter((p) => p.sku !== product.sku);
  portfolio.store_products = updatedProducts;
...
exports.portfolioBulkWrite = catchAsyncErrors(async (req, res, next) => {
  ...
  const storeIdsInPortfolio = Object.keys(groupedData).filter((key) => storeIds.includes(key));
  ...
  storeIdsInPortfolio.map(async (key) => {
    ...
    portfolio.store_products = portfolio.store_products.concat(groupedData[key]);
  ...
  const data = await Portfolio.aggregate([
    {
      $match: {
        ...
        store_id: store_id ? store_id : { $exists: true },
      },
    },
    // remove empty store_products
    {
