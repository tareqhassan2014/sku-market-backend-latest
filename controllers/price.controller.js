...
exports.updatePrice = catchAsyncErrors(async (req, res, next) => {
    ...
    if (doc) {
        ...
        product.weeklyPrice = getWeeklyAveragePrice(history);
...
function getTopBottomPrice(priceHistory) {
    ...
    bottomPrice.last2Weeks = Math.min(
        ...last2Weeks.map((price) => price.price)
    );
    bottomPrice.last30Days = Math.min(
        ...last30Days.map((price) => price.price)
    );
...
function getWeeklyAveragePrice(priceHistory) {
...
function getDailyPrice(priceHistory) {
    ...
    // if result more the 15 return first 15
    if (result.length > 14) {
